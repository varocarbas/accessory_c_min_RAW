...
void* assign_array(void* out_, void* in_, const size_t size_, const type type_)
{
	...
	{
		for (size_t i = 0; i < size_; i++) { out_ = _assign_add_update_2d_array_internal(out_, in_, WRONG_POINTER, i, type_, WRONG_ACTION, TRUE, FALSE); }
	}
...
void* __assign_array(void* in_, const size_t size_, const type type_) { return (array_is_ok(in_, size_, type_) == TRUE ? __assign_array_internal(WRONG_POINTER, in_, size_, type_, FALSE, FALSE) : __get_wrong_array_heap()); }
...
void* __reduce_array_size(void* in_, const size_t size_, const type type_, const boolean free_in_)
{
	...
	for (size_t i = 0; i < size_; i++)
	{
		...
		if (is_2d == TRUE)
		{
			value = __assign_void(get_2d_array_item(in_, i, type_), type_);

			out = _add_update_2d_array_internal(out, value, i, type_, ADD);
		}
		else
		{
			value = __assign_void(get_1d_array_item(in_, i, type_), type_);

			out = add_update_1d_array_internal(out, value, i, type_, ADD);
		}
...
void* get_1d_array_item(void* in_, const size_t i_, const type type_)
{
	void* out;
...
void* get_2d_array_item(void* in_, const size_t i_, const type type_)
{
	void* out;
...
type get_array_type_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_type(get_1d_array_item(in_, i_, TYPE)) : WRONG_TYPE); }

type_error get_array_error_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_error(get_1d_array_item(in_, i_, ERROR)) : WRONG_ERROR); }

type_warning get_array_warning_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_warning(get_1d_array_item(in_, i_, WARNING)) : WRONG_WARNING); }

error_warning* get_array_error_warning_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_error_warning(get_2d_array_item(in_, i_, ERROR_WARNING)) : WRONG_ERROR_WARNING); }

output* get_array_output_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_output(get_2d_array_item(in_, i_, OUTPUT)) : WRONG_OUTPUT); }

boolean get_array_boolean_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_boolean(get_1d_array_item(in_, i_, BOOLEAN)) : WRONG_BOOLEAN); }

char* get_array_string_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_string(get_2d_array_item(in_, i_, STRING)) : WRONG_STRING); }

char get_array_char_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_char(get_1d_array_item(in_, i_, CHAR)) : WRONG_CHAR); }

int get_array_int_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_int(get_1d_array_item(in_, i_, INT)) : WRONG_INT); }

size_t get_array_size_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_size(get_1d_array_item(in_, i_, SIZE)) : WRONG_SIZE); }

short get_array_short_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_short(get_1d_array_item(in_, i_, SHORT)) : WRONG_SHORT); }

long get_array_long_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_long(get_1d_array_item(in_, i_, LONG)) : WRONG_LONG); }

double get_array_double_value(void* in_, const size_t i_) { return (void_is_ok(in_) == TRUE ? void_to_double(get_1d_array_item(in_, i_, DOUBLE)) : WRONG_DOUBLE); }
...
void* __assign_array_internal(void* out_, void* in_, const size_t size_, const type type_, const boolean free_out_, const boolean free_in_)
{
	...
	else if (type_array_is_2d(type_) == TRUE)
	{
		...
		for (size_t i = 0; i < size_; i++) { out_ = _assign_add_update_2d_array_internal(out_, in_, WRONG_POINTER, i, type_, WRONG_ACTION, TRUE, TRUE); }
	}
...
void free_array_internal(void* in_h_, const size_t i_, const type type_) { if (in_h_ != WRONG_POINTER) free_void((type_array_is_2d(type_) ? get_2d_array_item(in_h_, i_, type_) : get_1d_array_item(in_h_, i_, type_)), type_); }

void* _assign_add_update_2d_array_internal(void* out_, void* in_, void* value_, const size_t i_, const type type_, const action action_, const boolean is_assign_, const boolean is_heap_)
{
	void* out = get_2d_array_item(out_, i_, type_);

	void* in;
	if (is_assign_ == TRUE) in = __assign_void(get_2d_array_item(in_, i_, type_), type_);

	void* value;
	if (is_assign_ == TRUE) value = (is_heap_ == TRUE ? __assign_void(in, type_) : assign_void(out, in, type_));
	else value = (action_ == ADD ? __assign_void(value_, type_) : update_void_value(out, value_, type_));

	if (type_ == ERROR_WARNING) ((error_warning**)out_)[i_] = void_to_error_warning(value);
	else if (type_ == OUTPUT) ((output**)out_)[i_] = void_to_output(value);
	else if (type_ == STRING) ((char**)out_)[i_] = void_to_string(value);

	if (is_assign_ == TRUE) free_void(in, type_);
...
void* add_update_1d_array_internal(void* array_, void* value_, const size_t i_, const type type_, const action action_)
{
	...
	if (type_ == TYPE) ((type*)array_)[i_] = void_to_type(action_ == ADD ? value : update_void_value(get_1d_array_item(array_, i_, type_), value, type_));
	else if (type_ == ERROR) ((type_error*)array_)[i_] = void_to_error(action_ == ADD ? value : update_void_value(get_1d_array_item(array_, i_, type_), value, type_));
	else if (type_ == WARNING) ((type_warning*)array_)[i_] = void_to_warning(action_ == ADD ? value : update_void_value(get_1d_array_item(array_, i_, type_), value, type_));
	else if (type_ == BOOLEAN) ((boolean*)array_)[i_] = void_to_boolean(action_ == ADD ? value : update_void_value(get_1d_array_item(array_, i_, type_), value, type_));
	else if (type_ == CHAR) ((char*)array_)[i_] = void_to_char(action_ == ADD ? value : update_void_value(get_1d_array_item(array_, i_, type_), value, type_));
	else if (type_ == INT) ((int*)array_)[i_] = void_to_int(action_ == ADD ? value : update_void_value(get_1d_array_item(array_, i_, type_), value, type_));
	else if (type_ == SIZE) ((size_t*)array_)[i_] = void_to_size(action_ == ADD ? value : update_void_value(get_1d_array_item(array_, i_, type_), value, type_));
	else if (type_ == SHORT) ((short*)array_)[i_] = void_to_short(action_ == ADD ? value : update_void_value(get_1d_array_item(array_, i_, type_), value, type_));
	else if (type_ == LONG) ((long*)array_)[i_] = void_to_long(action_ == ADD ? value : update_void_value(get_1d_array_item(array_, i_, type_), value, type_));
	else if (type_ == DOUBLE) ((double*)array_)[i_] = void_to_double(action_ == ADD ? value : update_void_value(get_1d_array_item(array_, i_, type_), value, type_));
...
void* _add_update_2d_array_internal(void* array_, void* value_, const size_t i_, const type type_, const action action_)
{
	...
	array_ = _assign_add_update_2d_array_internal(array_, WRONG_POINTER, value, i_, type_, action_, FALSE, FALSE);
...
char* _print_array_to_string_internal(void* in_, const size_t size_, const type type_, const boolean is_print_)
{
	...
	{
		void* value = __assign_void((type_array_is_2d(type_) == TRUE ? get_2d_array_item(in_, i, type_) : get_1d_array_item(in_, i, type_)), type_);

		if (is_print_ == TRUE) print_array_item_internal(i, max_i, value, get_type_format(type_), type_);
