...
output* __get_new_output(void* value_, const size_t size_, const type type_)
{
	...
	output temp = { (!is_array ? __assign_void(value_, type_) : __assign_array(value_, size_, type_)), size_, type_, TRUE, WRONG_ERROR_WARNING };

	output* out = __initialise_custom(&temp, OUTPUT);

	if (output_is_ok_internal(out, FALSE) && (value_ == WRONG_POINTER || type_ == WRONG_TYPE)) out = __update_output_error_warning_error(out, ERROR_WRONG_INPUTS, WRONG_POINTER);
...
output* __get_new_output_array(void* array_, const size_t size_, const type type_, const boolean free_variable_)
{
	...
	if (free_variable_) free_array(array_, size_, type_);
...
{
	if (output_is_ok_internal(in_h_, FALSE))
	{
...
{
	if (output_is_ok_internal(out_, FALSE) && output_is_ok_internal(value_, FALSE))
	{
		*out_ = *value_;
...
boolean output_variable_is_ok(output* in_) { return (output_is_ok_internal(in_, FALSE) && in_->_is_ok); }

boolean output_array_variable_is_ok(output* in_) { return (output_variable_is_array(in_) && array_is_ok(in_->_variable, in_->_size, in_->_type)); }

boolean output_variable_is_array(output* in_) { return (output_is_ok_internal(in_, FALSE) && output_variable_is_array_size(in_->_size)); }
...
{
	type out = (output_variable_is_ok(in_) ? void_to_type(in_->_variable) : WRONG_TYPE);

	if (free_in_) free_output(in_);
...
{
	type_error out = (output_variable_is_ok(in_) ? void_to_error(in_->_variable) : WRONG_ERROR);
...
{
	type_warning out = (output_variable_is_ok(in_) ? void_to_warning(in_->_variable) : WRONG_WARNING);
...
{
	error_warning* out = (output_variable_is_ok(in_) ? void_to_error_warning(in_->_variable) : WRONG_ERROR_WARNING);
...
{
	output* out = (output_variable_is_ok(in_) ? void_to_output(in_->_variable) : WRONG_OUTPUT);
...
{
	boolean out = (output_variable_is_ok(in_) ? void_to_boolean(in_->_variable) : WRONG_BOOLEAN);
...
{
	char* out = (output_variable_is_ok(in_) ? void_to_string(in_->_variable) : WRONG_STRING);
...
{
	char out = (output_variable_is_ok(in_) ? void_to_char(in_->_variable) : WRONG_CHAR);
...
{
	int out = (output_variable_is_ok(in_) ? void_to_int(in_->_variable) : WRONG_INT);
...
{
	size_t out = (output_variable_is_ok(in_) ? void_to_size(in_->_variable) : WRONG_SIZE);
...
{
	short out = (output_variable_is_ok(in_) ? void_to_short(in_->_variable) : WRONG_SHORT);
...
{
	long out = (output_variable_is_ok(in_) ? void_to_long(in_->_variable) : WRONG_LONG);
...
{
	double out = (output_variable_is_ok(in_) ? void_to_double(in_->_variable) : WRONG_DOUBLE);
...
char* __output_to_string(output* in_)
{
	...
	if (output_is_ok_internal(in_, TRUE))
	{
		if (!error_warning_is_ok_internal(in_->_error_warning, TRUE))
		{
...
char* _output_variable_to_string(output* in_)
{
	...
	if (output_variable_is_array(in_)) out = __output_array_variable_to_string_internal(in_);
	else if (output_is_ok_internal(in_, FALSE)) out = _void_type_to_string(in_->_variable, in_->_type);
...
char* __output_variable_to_string_full(output* in_)
{
	...
	if (is_array) type = __concatenate_two_strings(type, " array");

	char* items[] = { value, " ", DEFAULT_ENCLOSURE_INTERNAL_START, type, DEFAULT_ENCLOSURE_INTERNAL_END };
	out = __concatenate_strings(items, 5);

	if (is_array || void_type_to_string_is_heap(in_->_type))
	{
		free_string(value);

		if (is_array) free_string(type);
	}
...
{
	if (!output_is_ok_internal(out_, FALSE)) return out_;
...
output* __update_output_error_warning_conversion(output* out_, void* from_, const type type_from_, const type type_to_)
{
	...
	if (void_type_to_string_is_heap(type_from_)) free_string(temp);
...
{
	if (output_is_ok_internal(out_, FALSE) && output_is_ok_internal(in_, FALSE))
	{
		out_->_variable = (!output_variable_is_array(in_) ? __assign_void(in_->_variable, in_->_type) : __assign_array(in_->_variable, in_->_size, in_->_type));
...
boolean output_variables_are_equal_internal(output* in1_, output* in2_)
{
	...
	{
		if (is_array1) out = arrays_are_equal(in1_->_variable, in1_->_size, in2_->_variable, in2_->_size, in1_->_type);
...
output* __get_wrong_output_internal(const type type_, const type_error error_, const type_warning warning_, const boolean is_error_, void* further_)
{
	...
	if (output_is_ok_internal(out, FALSE)) out = _update_output_error_warning_internal(out, error_, warning_, is_error_, further_);
