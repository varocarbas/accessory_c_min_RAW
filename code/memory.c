...
void* __initialise_custom(void* in_, const type type_)
{
	...
	if (!void_type_is_ok(in_, type_)) out = __get_wrong_void(type_);
...
{
	free_(out_h_, (is_array_ ? tot_items_ : WRONG_SIZE), type_);
...
void* __get_wrong(const type type_, const boolean is_array_)
{
	...
	else if (type_ == STRING) out = __get_wrong_string();
	else out = _assign_internal(__initialise(1, type_, is_array_), get_wrong_internal(type_, is_array_), 1, type_, is_array_);
...
boolean pointer_is_ok(void* in_) { return (in_ != WRONG_POINTER); }
...
size_t get_memory_size(const size_t tot_items_, const type type_, const boolean is_2d_pointer_, const boolean is_heap_)
{
    ...
    if (type_ == STRING && !is_2d_pointer_) out++;
...
void* __assign_free_internal(void* in_, const size_t tot_items_, const type type_, const boolean is_array_, const boolean free_in_)
{
	...
	out = ((pointer_is_ok(out)) ? _assign_internal(out, in_, size, type_, is_array_) : __get_wrong(type_, is_array_));

	if (pointer_is_ok(out))
	{
		if (type_is_custom(type_)) out = __assign_free_custom_internal(out, in_, (is_array_ ? tot_items_ : WRONG_SIZE), type_);
	}
...
void* __assign_free_custom_internal(void* out_, void* in_, const size_t tot_items_, const type type_)
{
	...
	else if (!array_is_ok(out_, tot_items_, type_) || !array_is_ok(in_, tot_items_, type_)) return out_;
...
void* get_wrong_internal(const type type_, const boolean is_array_)
{
	...
	if (is_array_ || type_ == WRONG_TYPE) out = WRONG_POINTER;
